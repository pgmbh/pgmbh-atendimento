      ...
      <div class="dashboard-content" :style="{ marginLeft: sidebarCollapsed ? '60px' : '250px' }">
        <div class="config-page">
          <div class="d-flex justify-space-between align-center mb-4">
            <h2 class="text-h5 font-weight-medium">Tipos de Demanda</h2>
            <v-btn color="primary" @click="openNew" prepend-icon="mdi-plus">Novo Tipo</v-btn>
          </div>
          ...
          <v-card class="mb-4">
            <v-card-text>
              <v-row>
                ...
                <v-col cols="12" sm="3">
                  <v-select v-model="filterActive" :items="activeOptions" item-title="text" item-value="value" label="Status" hide-details />
                </v-col>
              </v-row>
            </v-card-text>
          </v-card>
    ...
    <v-dialog v-model="dialog.show" max-width="500px">
      <v-card>
        <v-card-title class="headline primary text-white">
          {{ dialog.isEdit ? 'Editar Tipo de Demanda' : 'Novo Tipo de Demanda' }}
        </v-card-title>
        <v-card-text class="pt-4">
          <v-form ref="form">
            <v-text-field v-model="formData.name" label="Nome *" :rules="[v => !!v || 'Nome é obrigatório']" required />
            <v-text-field v-model="formData.description" label="Descrição" class="mt-2" />
          </v-form>
        </v-card-text>
        <v-divider />
        <v-card-actions>
...
async function load() {
  loading.value = true
  try {
    const res = await configService.list('service-types', { active: filterActive.value })
    items.value = res.data.data
  } catch {
    showFeedback('Erro ao carregar tipos de demanda.', 'error')
  } finally {
    loading.value = false
  }
}
...
async function save() {
  ...
  try {
    if (dialog.value.isEdit) {
      await configService.update('service-types', formData.value.id, { name: formData.value.name, description: formData.value.description })
      showFeedback('Tipo de demanda atualizado com sucesso.')
    } else {
      await configService.create('service-types', { name: formData.value.name, description: formData.value.description })
      showFeedback('Tipo de demanda criado com sucesso.')
    }
...
async function toggle(item) {
  try {
    const res = await configService.toggleActive('service-types', item.id)
